...
Execute using: (show help with: ./packetSniper )
--------------
./packetSniper trace_filename 
or 
./packetSniper trace_filenam -arg1 -arg2 -...

Ex. ./packetSniper filename: shows the total number of packets of the trace file 'filename'.
...
void show_menu(){
	...
	printf("----------------------------------------------------\n");
	printf("./packetSniper trace_filename -argument1 -argument2 -....\n");
	printf("./packetSniper filename: shows the total number of packets of the trace file 'filename'.\n");
	printf("----------------------------------------------------\n");
	...
	printf("-open_ports: to create the files 'open_ports_found.txt' that shows all open ports for connection to the destination discovered"); 		printf(", and 'tcpflags_used.txt' file that shows all raised tcp flags for each packet.\n");
...
void determine_tcp_flags(int flags,int num_of_packets,FILE * fp,FILE *fp1,int port){
...
int main(int argc,char *argv[]){
	...
	for (argp=2;argp<argc;argp++){
		...
		if (strcmp(argv[argp],"-open_ports")==0){
			tcpflags_flag=1;
			tcp_flags = fopen ( "tcpflags_used.txt", "w" ) ;
			open_ports = fopen ("open_ports_found.txt","w");
			if (tcp_flags == NULL) {
				printf ("TCP flags File not created!!\n");
				exit(1);
		    	}
			if (open_ports == NULL) {
				printf ("Open ports File not created!!\n");
				exit(1);
		    	}
		}
	...
	while ((packet = pcap_next(pcap,&header)) != NULL){
		...
		/* determine tcp flags */
		if (tcpflags_flag){
			determine_tcp_flags(tcp->th_flags,num_of_packets,tcp_flags,open_ports,ntohs(tcp->th_sport));
		}
